...
async def get_twitter_stats(session, tweet_url):
    """Запрос статистики из API SocialData с таймаутом"""
    ...
    try:
        # Тайм-аут 10 секунд, чтобы скрипт не висел
        async with session.get(url, headers=headers, timeout=10) as resp:
            ...
            return None # Пропускаем 403, 404 и прочие молча
    except Exception:
        return None
...
async def main():
    log("Запуск коллектора (Версия: Fast Production)...")
    users = {}
    user_tweets = {} 
    ...
    async with aiohttp.ClientSession() as session:
                ...
                for link in unique_links:
                    ...
                    if not stats:
                        ...
                        if stats:
                            ...
                            # Платный тариф: задержка минимальна
                    ...
                    if stats:
                        ...
                        # Самостоятельный расчет (как страховка для XP)
        ...
        log(">>> ШАГ 4: Синхронизация XP из ветки бота...")
    ...
    for uid, info in users.items():
        # Если XP в боте не найдено, ставим 1 сообщение = 10 XP
    ...
    if payload:
        log(f">>> СИНХРОНИЗАЦИЯ: {len(payload)} пользователей в Supabase...")
